...
- This will create a project folder and inside it a `src` folder, a `.gitignore` file, and a `Cargo.toml`<sup>1</sup> file
...
cargo build
```
- we get a new folder called `target` and a `Cargo.lock`<sup>2</sup> file
...
6. A **macro** is a compile-time code generator that takes Rust source code as input and produces expanded Rust source code as output. Think of it as a **syntax-level transformer**: while a function manipulates _values_, a macro manipulates _the code itself_ before the program is finished being built.
